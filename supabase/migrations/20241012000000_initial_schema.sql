...
-- Username is auto-generated from Google OAuth, but can be edited

CREATE TABLE IF NOT EXISTS public.profiles (
  id UUID REFERENCES auth.users(id) ON DELETE CASCADE PRIMARY KEY,
  username TEXT NOT NULL, -- Auto from Google full_name or email
  display_name TEXT, -- Optional custom display name
  avatar_url TEXT, -- From Google OAuth picture field
  created_at TIMESTAMPTZ DEFAULT NOW() NOT NULL,
  updated_at TIMESTAMPTZ DEFAULT NOW() NOT NULL,

  -- Username should be unique but we'll handle collisions in trigger
...
-- MVP: All rooms are PRIVATE (share code required to join)
...
CREATE TABLE IF NOT EXISTS public.rooms (
  ...
  description TEXT,
  max_members INT DEFAULT 10 NOT NULL, -- Reasonable limit
  created_by UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  created_at TIMESTAMPTZ DEFAULT NOW() NOT NULL,
  last_activity_at TIMESTAMPTZ DEFAULT NOW() NOT NULL,
  is_archived BOOLEAN DEFAULT FALSE NOT NULL,
  archived_at TIMESTAMPTZ -- When room was archived
);
...
-- Users can only view rooms they're members of (all rooms are private)
...
-- Users can join rooms (share code validation enforced in app/API layer)
...
-- Pulls name/avatar from Google OAuth metadata
...
-- ============================================
-- AUTO-ARCHIVE ROOMS WHEN GAME ENDS
-- ============================================
...
BEGIN
  ...
  IF NEW.actual_end IS NOT NULL AND OLD.actual_end IS NULL THEN
    -- Check if ALL games in this room are now finished
    IF NOT EXISTS (
      ...
      WHERE id = NEW.room_id AND is_archived = FALSE;
    END IF;
  END IF;
